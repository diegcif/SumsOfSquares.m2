...
An element $f\!\in\! R$ is a \emph{sum-of-squares} or \emph{SOS} if there are polynomials $f_{1},\dots,f_{m} \!\in\! R$ and positive scalars $\lambda_{1},\dots,\lambda_{m} \!\in\! \kk$ such that $f=\sum_{i}\lambda_i f_{i}^{2}$.
...
Let $f(x)\in R$ of degree~$2d$,
and $v$ a vector with all the $N=\binom{n+d}{d}$ monomials of degree $\leq\!d$.
The following fundamental result holds:
\begin{align*}
  f(x) \text{ is SOS } 
  \qquad\iff\qquad
  \exists\, Q\in \mathbb{S}_+^N \text{ such that } f(x) = v^T Q v,
\end{align*}
...
We use it to verify that $f = 2 x^4 + 5 y^4 - 2 x^2 y^2 + 2 x^3 y$ is a sum-of-squares.
%The input is the polynomial $f(x)$.
{\small
\begin{verbatim}
i1 : R = QQ[x,y];
...
\noindent
The last line indicates that a Gram matrix was found, so $f$ is indeed~SOS.
...
In the above example there are three squares:
\[
...
We consider an example from~\cite{parrilo2005exploiting}:
proving that $f = 10{-}x^2{-}y$ is nonnegative on the circle defined by $g = x^2 {+} y^2 {-} 1$.
...
In the next example we set $2d=2$.
{\small
\begin{verbatim}
...
As an illustration, we do this computation for the ideal $I=\langle x^2{+}y^2{+}y, y{-}z^2 \rangle$.
{\small
\begin{verbatim}
i1 : R = QQ[x,y,z];
...
This option tells the SDP solver to minimize the trace of the Gram matrix.
