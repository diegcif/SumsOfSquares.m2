...
The package contains a small library of interesting nonnegative forms available under \verb|nonnegativeForm|.
We use it here.
{\small
\begin{verbatim}
i2 : R = QQ[x,y,z]
...
\begin{verbatim}
i1 : R = QQ[x,z];
i2 : f = nonnegativeForm ("Motzkin", {x,1,z});
...
Though the above method is very appealing from a mathematical perspective, 
constructing the quotient ring is sometimes very computationally expensive (it requires Gr\"obner bases).
An alternative is to call \verb|lowerBound| with the equations $h_1,\dots,h_m$ as the input.
...
In some cases, the minimizer might also be recovered from SDP solution with the method \verb|recoverSolution|.
{\small
\begin{verbatim}
