...
\noindent
Although the result of the SDP solver is in floating point, Macaulay2 rounds it to obtain a rational Gram matrix~\cite{peyrl2008computing}.
The rounding procedure is guaranteed to work when the floating point Gram matrix lies in the interior of~$\mathbb{S}_+^N$.
...
We briefly describe the rounding procedure from~\cite{peyrl2008computing},
implemented in the \SOS package.
Consider the problem of finding a rational SOS decomposition for a polynomial $f\in \QQ[x_1,\dots,x_n]$.
The most important step is computing a Gram matrix~$Q$ with rational entries.
Given such a matrix, we can recover the SOS decomposition via an LDL factorization.

We need to find a rational matrix in $\mathcal{L}\cap \mathbb{S}_+^N$,
where $\mathcal{L}\subset \mathbb{S}^N$ is the affine space $\{Q : v^T\! Q v \!=\! f\}$.
The SDP solver gives a numerical matrix $Q_0\in\mathbb{S}_+^N$ that lies close to~$\mathcal{L}$.
Let $Q_1$ be a rational matrix obtained by rounding the entries of~$Q_0$.
Let $Q_2$ be the projection of $Q_1$ onto~$\mathcal{L}$.
The matrix $Q_2$ lies in~$\mathcal{L}$ and has rational entries, 
so it remains to see if it is positive semidefinite. 
This can be ensured as long as $Q_0$ lies in the interior of $\mathbb{S}_+^N$;
see \cite[Prop.~8]{peyrl2008computing}.
More precisely, we need to choose a small enough {infeasibility tolerance}~$\delta:= \textrm{dist}(Q_0,\mathcal{L})$
and {rounding tolerance}~$\tau:= \textrm{dist}(Q_0,Q_1)$,
so that the smallest eigenvalue of~$Q_0$ is greater than $\sqrt{\delta^2 {+} \tau^2}$.
