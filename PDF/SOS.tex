...
Let $f\in \QQ[x_1,\dots,x_n]$ and consider the affine space $\mathcal{L}:= \{Q: v^T\! Q v \!=\! f\}$.
% \subset \mathbb{S}^N$.
...
First, the entries of~$Q_n$ are rounded to a rational matrix~$Q_r'$, which usually leaves $\mathcal{L}$.
...
then $Q_r\in \mathbb{S}_+^N$ can be ensured using appropriate parameters.
Given the \emph{infeasibility tolerance}~$\delta:= \textrm{dist}(Q_n,\mathcal{L})$, if the \emph{rounding tolerance}~$\tau:= \textrm{dist}(Q_n,Q_r')$ is so that $\sqrt{\delta^2 {+} \tau^2}$ is less than the smallest eigenvalue of~$Q_n$, then \cite[Prop.~8]{peyrl2008computing} guarantees that $Q_r \in \mathbb{S}_+^N$.
}
