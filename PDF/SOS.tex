...
Note that $Q_n$ may not lie exactly on~$\mathcal{L}$.
...
We may ensure that $Q_r\in \mathbb{S}_+^N$ if the numerical matrix $Q_n$ is sufficiently in the interior of~$\mathbb{S}_+^N$.
More precisely, assume that $\lambda$, the smallest eigenvalue of~$Q_n$, is greater than the {infeasibility tolerance}~$\delta:= \textrm{dist}(Q_n,\mathcal{L})$.
...
% If the numerical matrix $Q_n$ is in the interior of~$\mathbb{S}_+^N$,
% then $Q_r\in \mathbb{S}_+^N$ might be ensured using appropriate parameters.
% Given the {infeasibility tolerance}~$\delta:= \textrm{dist}(Q_n,\mathcal{L})$, 
% if the {rounding tolerance}~$\tau:= \textrm{dist}(Q_n,Q_r')$ is so that $\sqrt{\delta^2 {+} \tau^2}$ is less than the smallest eigenvalue of~$Q_n$, 
% then \cite[Prop.~8]{peyrl2008computing} guarantees that $Q_r \in \mathbb{S}_+^N$.
}
