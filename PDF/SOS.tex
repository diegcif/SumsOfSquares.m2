...
\noindent
The result of the semidefinite programming solver is a floating point approximation of an optimal point.
The \SOS package attempts to find a close enough rational Gram matrix~\cite{peyrl2008computing}.
...
In an object of this type are stored the coefficients $\lambda_{i}$ (under \verb|coeffs|) and polynomials $f_{i}$ (under \verb|gens|) such that $f = \sum_{i}\lambda_{i}f_{i}^{2}$.
